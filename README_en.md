...
### Install the CLI and Skill (recommended)

Install the GitHub repository version from another workspace:
...
Agents installing from this repository must follow [AGENTS.md](AGENTS.md#cli-与-skill-安装) for PATH recovery and version verification.
...
- `~/.hermit/logs/hermit.log`: server log, rotated at 256 MiB with one backup (`hermit.log.1`); oversized legacy logs are trimmed to their newest complete lines before startup
...
See [AGENTS.md](AGENTS.md) for the development environment, tests, installation verification, and pre-PR checks. Implementation details live in [`docs/design.md`](docs/design.md).
