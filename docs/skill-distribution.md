...
2. **全局分发**：通过 CLI 子命令 `hermit install-skills` 将 Skill 安装到 `~/.agents/skills/`，供其他工作区复用。
...
3. 将每个含 `SKILL.md` 的 Skill 目录拷贝到 `~/.agents/skills/<skill-name>/`。
...
4. `hermit install-skills` 可返回成功 JSON，并正确复制到 `~/.agents/skills/`。
